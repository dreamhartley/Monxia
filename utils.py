...
def download_image_with_page(page: Page, url: str, artist_id: int, timeout: int = 30000) -> Optional[str]:
    """
    使用Playwright下载图片到本地
    page: Playwright页面对象
    url: 图片URL
    ...
    try:
        ...
        if image_content[:8] == b'\x89PNG\r\n\x1a\n':
            ext = 'png'
        elif image_content[:3] == b'\xff\xd8\xff':
            ext = 'jpg'
        ...
        # 如果已存在同ID的其他格式图片，先删除
        for old_file in IMAGES_DIR.glob(f"{artist_id}.*"):
...
def get_example_image(page: Page, artist_id: int, max_retries: int = 5) -> Optional[str]:
    """
    ...
    try:
        ...
        try:
            for attempt in range(max_retries):
                ...
                filename = download_image_with_page(download_page, original_url, artist_id)
                ...
                if filename:
                    ...
                    if original_url.lower().endswith('.jpg'):
                        ...
                        filename = download_image_with_page(download_page, png_url, artist_id)
...
def get_post_count_and_image(page: Page, url: str, artist_id: int, artist_name: str, retry_count: int = 3) -> Dict[str, any]:
    """
    ...
    for attempt in range(retry_count):
        try:
            ...
            # 获取示例图(下载到本地)
            example_image = get_example_image(page, artist_id)
...
def fetch_post_counts_batch(artists: list) -> dict:
    """
    批量获取画师作品数量和示例图
    artists: 列表,每个元素是字典 {'id': ..., 'danbooru_link': ..., 'name': ...}
    ...
    with sync_playwright() as playwright:
        ...
        for artist in artists:
            ...
            result = get_post_count_and_image(page, url, artist_id, name)
